...
The simulation demonstrates critical risks in container orchestration, specifically focusing on the dangers of exposing Unix sockets (`/var/run/docker.sock`) to unprivileged containers.
...
*Figure 3: Successfully executing `docker ps` from inside the restricted container, proving control over the host daemon.*
...
*Figure 4: Running the `recovery_script.sh` to restore the application to its original state.*
...
*Figure 5: Proof of root access and successful system recovery (`THM{DOCKER_ESCAPE_SUCCESS}`).*
...
3.  **Rootless Docker:** Run the Docker daemon as a non-root user. This limits the potential damage if an attacker successfully escapes a container.
4.  **Least Privilege:** Ensure containers run with the minimum necessary permissions and avoid using the `--privileged` flag.
...
* **Audit Trails:** Regular audits of `docker inspect` configurations are essential to catch dangerous mounts.
